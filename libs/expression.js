...
function calcs(input, xrange, yrange) {
    ...
    if (xrange[0] == xrange[1]) {
        return parser(xrange[0]);
    }
    if (input !== unlastInput || xrange.toString() !== oldxrange.toString() || yrange.toString() !== oldyrange.toString()) {
        oldxrange = xrange;
        oldyrange = yrange;
        rpndata.length = 0;
        rt.length = 0
        ...
        if (result == "Syntax Error") {    //ERROR
            ...
            do {
                for (var xx = xrange[0]; result[1].x != -1 && xx < xrange[1]; xx += dealt) {
                    yyy = parser(xx);
                    //maybe return NaN, Even if you get a NaN, also still push it and draw function will deal with this case;
                    rt.push([xx, yyy]);
                    /*
                    if (!isNaN(yyy)) {
            ...
            var xmM = [xrange[0], xrange[1]], ymM = [xrange[0], xrange[1]];
            var dealt = (xrange[1] - xrange[0]) / 100;
            if (dealt <= 0.1) {
                dealt = 0.1
            } else if (dealt <= 0.5) {
